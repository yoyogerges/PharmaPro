...
const shared_1 = require("@pharmapro/shared");
...
async function seedPermissions() {
    ...
    await prisma.permission.createMany({
        data: shared_1.PERMISSION_DEFINITIONS.map((p) => ({
            module: p.module,
            action: p.action,
    ...
    console.log(`seeded ${shared_1.PERMISSION_DEFINITIONS.length} permissions`);
}
async function seedRoles() {
    for (const role of shared_1.DEFAULT_ROLES) {
        const permissionSlugs = shared_1.ROLE_PERMISSIONS[role.name] ?? [];
    ...
    console.log(`seeded ${shared_1.DEFAULT_ROLES.length} roles`);
}
...
async function seedStaffAccountsAndCatalog() {
    const staffAccounts = [
    ...
    for (const account of staffAccounts) {
        ...
        if (!role)
            throw new Error(`role ${account.role} missing`);
...
async function seedSuppliersAndCustomers() {
    const supplierNames = [
...
async function seedExtraCategories() {
...
async function ensureCategoryId(name, nameAr, sortOrder) {
    let category = await prisma.productCategory.findFirst({ where: { name } });
    if (!category)
        category = await prisma.productCategory.create({ data: { name, nameAr, sortOrder } });
    return category.id;
}
async function ensureManufacturerId(name, country) {
    let manufacturer = await prisma.manufacturer.findFirst({ where: { name } });
    if (!manufacturer)
        manufacturer = await prisma.manufacturer.create({ data: { name, country } });
...
async function seedGiantCatalog() {
